        ...
        if (isset($_POST['submit'])) {
                ...
                if ($user->login == $_POST['login'] && $user->password == MD5($_POST['password'])) {
                    ...
                    if ($user->role == 'Administrateur') {
                        ...
                        if ($user->login == 'hiba' || $user->login == 'badreddine') {
                            header('location: rams');
                        } else {
                            header('location: ptmo-ppmo');
                        }
                    } elseif ($user->role == 'Consultant') {
                        if ($user->login == 'meryem' || $user->login == 'wiam') {
                            header('location: ptmo-ppmo');
                        } elseif ($user->login == 'hamza' || $user->login == 'touria' || $user->login == 'abdessalam') {
                            header('location: rams');
                        }
                    }
                } else {
